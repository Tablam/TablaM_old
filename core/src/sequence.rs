...
impl Relation for Seq {
    ...
    fn filter(&self, cmp: CmOp) -> Rel {
        //        let iter = self.iter.filter(|tuple| true);
        //        Seq::new(self.schema.clone(), &self.shape, Box::new(iter)).into()
        unimplemented!()
    }
    ...
    fn union(&self, other: &Rel) -> Rel {
        //        match other {
        //            Rel::Seq(b) => {
        //                let iter = self.iter.chain(b.iter.into_iter());
        //                Seq::new(self.schema.clone(), &self.shape, Box::new(iter)).into()
        //            }
...
impl Seq {
    pub fn new(schema: Schema, shape: &Shape, iter: Box<dyn Iterator<Item = Col>>) -> Self {
        Seq {
            iter,
            schema,
            shape: *shape,
